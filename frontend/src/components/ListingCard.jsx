...
const timeLeft = (exp) => {
  ...
  if (diff <= 0) {
    return { label: 'Expired', cls: 'timer-exp' };
  }
...
export default function ListingCard({ listing, onUpdate }) {
  ...
  const claim = async () => {
    ...
    try {
      ...
      alert(e.response?.data?.message || 'Could not claim');
    } finally {
      setLoading(false);
    }
  };
  ...
  const isMyClaim =
    user &&
    listing.claimedBy &&
    listing.claimedBy._id === user._id;

  const isMyListing =
    user &&
    listing.donor &&
    (
      listing.donor._id === user._id ||
      listing.donor === user._id
    );
  ...
  return (
    <div className={`listing-card ${listing.status}`}>
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <div className="meta" style={{ marginTop: 3 }}>
              <strong style={{ color: 'var(--manit-green)' }}>
        ...
        <span
          className={`badge badge-${listing.status}`}
          style={{ flex: 'none' }}
        >
          {listing.status}
        </span>
      ...
      >
        ...
        <div className="meta">
          🍽{' '}
          <strong>
            {listing.donor?.name || 'Unknown'}
          </strong>

          {listing.donor?.phone && (
            <span style={{ color: 'var(--manit-cyan)' }}>
              {' '}
              · 📞 {listing.donor.phone}
            </span>
          )}
        ...
        {listing.claimedBy && (
          <div className="meta">
            🚴 Claimed by:{' '}
            <strong>{listing.claimedBy.name}</strong>
          </div>
        )}
      </div>
      ...
      >
        <span className={`timer ${timer.cls}`}>
          ⏱ {timer.label}
        </span>
      ...
      <div className="actions">
        {user &&
          ['volunteer', 'ngo'].includes(user.role) &&
